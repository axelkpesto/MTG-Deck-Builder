...
class CardDecoder:
    ...
    def __init__(self, embed_dim: int = 686):
        ...
        Args:
            embed_dim: Dimensionality of the text embedding suffix in the vector.
    ...
    def decode(self, card_name: str, encoded_vector: np.ndarray) -> str:
        ...
        Args:
            card_name: The card's canonical name.
            encoded_vector: The fixed-length numeric feature vector for the card.

        Returns:
            A newline-separated string of field: value pairs.
        """
    ...
    def _title_case(name: str) -> str:
        """Title-case a card name, keeping common prepositions lowercase.

        Args:
            name: Raw card name string.

        Returns:
            Title-cased card name.
        """
        _transition = frozenset({'of', 'the', 'in', 'on', 'at', 'to', 'for', 'and', 'but', 'or', 'nor', 'a', 'an'})
        words = name.split(' ')
        return ' '.join(
            w.lower() if (i > 0 and w.lower() in _transition) else w.capitalize()
    ...
    def decode_to_dict(self, card_name: str, encoded_vector: np.ndarray) -> Dict[str, str]:
        ...
        Returns:
            A dict mapping field names (Name, Types, Mana Cost, etc.) to display strings.
        """
        idx = 0
        ...
        return {
            "Name": self._title_case(card_name),
            ...
            "Color Identity": str([x.upper() for x in color_identity]),
            "Rarity": str(rarity).capitalize(),
        }

    def int_to_rarity(self, rarity_int: int) -> str:
        """Convert an encoded rarity integer to a rarity string.

        Args:
            rarity_int: Integer encoding of the rarity.

        Returns:
            Rarity string, or 'Unknown' if the integer is unrecognized.
        """
        return CardFields.rarity_map().get(rarity_int, 'Unknown')

    def slice(self, key: str, dim: int) -> slice:
        """Return the slice for a feature group given the full vector dimension.

        Args:
            key: Feature group name (e.g. 'types', 'colors', 'embed').
            dim: Total length of the vector.

        Returns:
            A slice object covering the requested feature group.
        """
        if key == "embed":
    ...
    def item_from_vector(self, vec: torch.Tensor, value: str, threshold: float = 0.5) -> List[str]:
        ...
        Args:
            vec: The full card feature vector.
            value: Feature group key to decode (e.g. 'types', 'colors').
            threshold: Activation threshold for binary features.

        Returns:
            List of active category labels for the requested feature group.
        """
    ...
    def constrain_logits(self, logits: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
        ...
        Args:
            logits: Raw output tensor of shape (..., vector_dim).
            threshold: Binarization threshold for categorical features.

        Returns:
            Constrained tensor of the same shape with valid feature encodings.
        """
        dim = logits.size(-1)
    ...
    def land_mask_from_vectors(self, node_features: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
        """Return a boolean mask of rows encoded as land cards.

        Args:
            node_features: Node feature matrix of shape (N, vector_dim).
            threshold: Activation threshold for the land type bit.

        Returns:
            Boolean tensor of shape (N,) where True indicates a land card.
        """
    ...
    def color_identity_mask_from_vectors(self, node_features: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
        """Return a boolean color-identity mask from encoded node features.

        Args:
            node_features: Node feature matrix of shape (N, vector_dim).
            threshold: Activation threshold for color identity bits.

        Returns:
            Boolean tensor of shape (N, num_colors) indicating active color identities.
        """
    ...
    def mana_value_from_vectors(self, node_features: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Float tensor of shape (N,) with mana values clamped to [0, 30].
        """
