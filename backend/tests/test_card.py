...
class TestCardConstructor:
    ...
    def test_negative_mana_cost_allowed(self):
        # Encoder/decoder may use -1 to signal "missing"; constructor must not reject it.
...
class TestCardEquality:
    ...
    def test_equal_when_card_id_matches(self):
        # Split-name / alternate-printing variants of the same card share a MultiverseID.
    ...
    def test_unequal_to_non_card_object(self):
        ...
        assert (c == "X") is False
        assert (c == None) is False  # noqa: E711  (explicit equality is the contract)
        assert (c == 42) is False
        assert (c == {"card_name": "X"}) is False
...
class TestCardHash:
    ...
    def test_equal_cards_hash_equal(self):
        # Same card_id → equal → must hash equally (eq/hash contract).
        a = make_card(card_name="Forest", card_id="mv-1")
        b = make_card(card_name="Forest // Forest", card_id="mv-1")
        assert a == b
...
class TestStr:
    ...
    def test_one_line_per_field(self):
        c = make_card()
        s = str(c)
        # Same number of lines as attributes
...
class TestCardInDictAndSet:
    ...
    def test_cards_with_same_id_collapse_in_set(self):
        # Same card_id ⇒ same hash AND equal ⇒ a set keeps only one.
