...
class TestBasicLandHelpers:
    ...
    def test_is_basic_land_false_for_non_basic(self, name):
        # Note: snow basics are NOT in the `BASIC_LANDS` set used by is_basic_land
    ...
    def test_basic_type_name_resolves_lowercase(self):
        # CardFields.basic_type_name returns the title-cased land type
...
class TestRarityMaps:
    ...
    def test_rarity_map_is_one_indexed(self):
        ...
        assert 0 not in m  # zero-indexed would put a value at 0
        assert 1 in m
...
class TestPredicateFactories:
    ...
    def test_pred_not_inverts_predicate(self):
        always_true = lambda n, v: True
        p = CardFields.pred_not(always_true)
        assert p("x", np.zeros(1)) is False

    def test_pred_all_returns_true_iff_every_inner_true(self):
        true_p = lambda n, v: True
        false_p = lambda n, v: False
        assert CardFields.pred_all(true_p, true_p)("x", np.zeros(1)) is True
        assert CardFields.pred_all(true_p, false_p)("x", np.zeros(1)) is False

    def test_pred_any_returns_true_if_any_inner_true(self):
        true_p = lambda n, v: True
        false_p = lambda n, v: False
        assert CardFields.pred_any(false_p, false_p)("x", np.zeros(1)) is False
        assert CardFields.pred_any(false_p, true_p)("x", np.zeros(1)) is True
